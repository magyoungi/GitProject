            ...
            for top in tops:
                if top!= "KRW-BTT" and top!= "KRW-T" and RSI(top)[-3] >= 70 and RSI(top)[-2] < 70 and red_volume(top)*0.5 < blue_volume(top):
                    if ma15() < ma50():
                        ...
                        post_message(myToken, "#mid10_beta2_rev0", "Start Short : " + top + " " + str(bprice))
                        y = 1
                        while y == 1:
                            ...
                            if cprice >= 1.015 * bprice:
                                post_message(myToken, "#mid10_beta2_rev0", "Short Fail: " + str(cprice))
                                break
                            elif cprice <= 0.985 * bprice:
                                post_message(myToken, "#mid10_beta2_rev0", "Short Sucess: " + str(cprice))
                                break
                    elif ma15() > ma50():
                        bprice = pyupbit.get_current_price(top)
                        post_message(myToken, "#mid10_beta2_rev0", "OMG Start Short : " + top + " " + str(bprice))
                        y = 1
                        while y == 1:
                            nowhour = datetime.datetime.now()
                            time.sleep((5 - nowhour.minute % 5) * 60)
                            cprice = pyupbit.get_current_price(top)
                            if cprice >= 1.015 * bprice:
                                post_message(myToken, "#mid10_beta2_rev0", "Told ya Short Fail: " + str(cprice))
                                break
                            elif cprice <= 0.985 * bprice:
                                post_message(myToken, "#mid10_beta2_rev0", "OMG Short Sucess: " + str(cprice))
                                break

        except Exception as e:
            post_message(myToken, '#mid10_beta2_rev0', e)
            time.sleep(30)

schedule.every(2).minutes.do(main)
post_message(myToken, "#mid10_beta2_rev0", "Start!!-----------------------------")
while True:
