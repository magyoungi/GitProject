        ...
        try:
            ...
            for top in tops:
                if top!= "KRW-BTT" and RSI(top)[-2] <= 30 and RSI(top)[-1] > 30:
                    ...
                    post_message(myToken, "#bottom8", "Start Long : " + top + " " + str(bprice))
                    y = 1
                    while y == 1:
                        ...
                        if cprice >= 1.03 * bprice:
                            post_message(myToken, "#bottom8", "Long Sucess: " + str(cprice))
                            break
                        elif cprice <= 0.97 * bprice:
                            post_message(myToken, "#bottom8", "Long Fail: " + str(cprice))
                            break
                elif top!= "KRW-BTT" and RSI(top)[-2] >= 70 and RSI(top)[-1] < 70:
                    bprice = pyupbit.get_current_price(top)
                    post_message(myToken, "#bottom8", "Start Short : " + top + " " + str(bprice))
                    y = 1
                    while y == 1:
                        nowhour = datetime.datetime.now()
                        time.sleep((5 - nowhour.minute % 5) * 60)
                        cprice = pyupbit.get_current_price(top)
                        if cprice >= 1.03 * bprice:
                            post_message(myToken, "#bottom8", "Short Fail: " + str(cprice))
                            break
                        elif cprice <= 0.97 * bprice:
                            post_message(myToken, "#bottom8", "Short Sucess: " + str(cprice))
                            break
        except Exception as e:
            post_message(myToken, '#bottom8', e)
            time.sleep(30)

schedule.every(2).minutes.do(main)
post_message(myToken, "#bottom8", "Start!!-----------------------------")
while True:
